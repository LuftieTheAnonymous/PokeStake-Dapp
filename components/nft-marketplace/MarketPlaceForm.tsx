      ...
      useWatchContractEvent({
        ...
        onLogs(logs){
          if((logs[0] as any).args.owner === walletAddress && (logs[0] as any).args.approved === marketPlaceAddress){
            setApproved(true);
            setLoading(false);
            toast("Approval commited successfully !");
          }
        }
      });
...
<div className={`relative gap-4 flex shadow-lg shadow-primary/60 flex-col items-center max-h-80 h-full justify-center px-4 py-8 rounded-lg border border-primary cursor-pointer transition-colors duration-150`}>
  {data && !isLoading && data.length > 0 && data.slice(pageStartIndex, pageStartIndex + 3).map((availableNft) => (
  <NFTCard nftId={availableNft.nftId} {...register('nftId')}
  onClick={()=>{setValue('nftId',(availableNft.nftId)
  )}} nft={availableNft.card} key={Number(availableNft.nftId)} 
  selected={watch('nftId') === availableNft.nftId}
  />
  ))}
...
{formState.errors.nftId?.message && <span className="text-sm text-red-500">⚠️ ERROR: {formState.errors.nftId?.message}</span>}
          </div>
          ...
          <div className="space-y-2">
            ...
            {usdEstimate && (
              ...
              {formState.errors.listingPrice?.message && <span className="text-sm text-red-500">⚠️ ERROR: {formState.errors.listingPrice?.message}</span>}
              {formState.errors.isPaidInEth?.message && <span className="text-sm text-red-500">⚠️ ERROR: {formState.errors.isPaidInEth?.message}</span>}
